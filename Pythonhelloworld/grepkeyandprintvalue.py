...
print(f"Checking file types '{myext}' with more than {limit} bytes in {mypath}")
...
# parse all files and send output to a file if conditions match
start = time.time()
nr_files = 0
        ...
        for file in files:
            if limit > 0:
                ...
                if size != None and size <= limit:
                    # do nothing
                    print(f"File {file} is below {limit}, going to next file")
                    continue
            ...
            if myext != "*":
                ...
                if file_extension != myext:
                    # do nothing
                    print(f"File {file} has extension {file_extension}, going to next file")
                    continue
...
print(f"Found {nr_files} files with {searchpattern}. Output to {myfiles}")
